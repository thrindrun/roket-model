...
f = [
    Fx/m - q*w + r*v - cu*u - (m_dot/m)*u;  
    Fy/m - r*u + p*w - cv*v - (m_dot/m)*v;
    Fz/m - p*v + q*u - cw*w - (m_dot/m)*w - g;
    (L - q*r*(Iz - Iy))/Ix - cp*p;
    (M - r*p*(Ix - Iz))/Iy - cq*q;
    (N - p*q*(Iy - Ix))/Iz - cr*r;
...
% **Denge noktaları**
x_equilibrium = [0; 0; 0.2; 0; 0; 0; 0; 0; 0];
...
A_eq = subs(A_eq, [m, m_dot, Ix, Iy, Iz, g, cu, cv, cw, cp, cq, cr], ...
